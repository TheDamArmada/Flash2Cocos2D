...
package by.blooddy.core.utils {
	...
	public class DefinitionFinder {
		 ...
		 * @private
		 */
		private static const TAG_METADATA:uint = 77;
		 ...
		 * @private
		 */
		private function findTag(doRewind:Boolean=false):uint {
			if (doRewind) this._data.rewind();
			var tag:uint;
			var id:uint;
			var length:uint;
			...
			while (this._data.bytesAvailable > 1) {
				...
				switch (id) {
					case DefinitionFinder.TAG_METADATA:
						this._metadata = this._data.readString();
						break;
